...
class ReverieServer: 
  ...
  def save(self): 
    ...
    self.export_agent_log()

  def export_agent_log(self):
    """
    Exports all agents' thoughts, actions, and conversations to a Markdown file.
    Each agent is a top-level header. Each activity entry is a sub-header with
    its timestamp, and the type and description branch off it as a tree.
    """
    sim_folder = f"{fs_storage}/{self.sim_code}"
    log_file = f"{sim_folder}/agent_log.md"

    lines = []
    generated_at = datetime.datetime.now().strftime("%B %d, %Y, %H:%M:%S")
    lines.append(f"# Simulation Log — {self.sim_code}")
    lines.append(f"")
    ...
    for persona_name, persona in self.personas.items():
      ...
      for thought in persona.a_mem.seq_thought:
        agent_records.append({
          'timestamp': thought.created,
          'type':      'Thought',
          'description': thought.description,
        })

      for event in persona.a_mem.seq_event:
        if event.subject == persona_name:
          agent_records.append({
      ...
      if not agent_records:
        ...
        for r in agent_records:
          ts = r['timestamp'].strftime("%B %d, %Y, %H:%M:%S")
          lines.append(f"## {ts}")
  ...
  def save_metrics(self):
    """
    Save current simulation metrics to a JSON file for the dashboard.
    Called at the end of each simulation step cycle.
    """
    sim_folder = f"{fs_storage}/{self.sim_code}"
    metrics = {
    ...
    for persona_name, persona in self.personas.items():
      metrics["personas"][persona_name] = {
        "mood": round(persona.scratch.mood, 3),
        "mood_baseline": round(persona.scratch.mood_baseline, 3),
        "mood_history": persona.scratch.mood_history,
        "convo_count_today": persona.scratch.convo_count_today,
        "relationship_scores": persona.scratch.relationship_scores,
        "memory_count": len(persona.a_mem.id_to_node),
        "thought_count": len(persona.a_mem.seq_thought),
        "event_count": len(persona.a_mem.seq_event),
        "chat_count": len(persona.a_mem.seq_chat),
        "current_action": persona.scratch.act_description if persona.scratch.act_description else "idle",
      }
