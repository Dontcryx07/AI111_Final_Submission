...
def new_retrieve(persona, focal_points, n_count=30): 
  ...
  for focal_pt in focal_points: 
    ...
    # =============================================
    # MEMORY DECAY / FORGETTING (NEW)
    # =============================================
    # Filter out old, unimportant, unaccessed memories.
    # Keep a memory if ANY of these conditions are true:
    #   1. It was accessed in the last 7 days (168 hours)
    #   2. It has high poignancy (importance >= 5 out of 10)
    #   3. It is a thought/insight (depth > 0) - derived knowledge is always kept
    if persona.scratch.curr_time and len(nodes) > 0:
      filtered_nodes = []
      for node in nodes:
        age_hours = (persona.scratch.curr_time - node.last_accessed).total_seconds() / 3600.0
        if age_hours < 168:          # Condition 1: accessed recently
          filtered_nodes.append(node)
        elif node.poignancy >= 5:    # Condition 2: important memory
          filtered_nodes.append(node)
        elif node.depth > 0:         # Condition 3: it's a thought
          filtered_nodes.append(node)
        # else: this memory is effectively "forgotten"
      # Safety: never forget EVERYTHING - use unfiltered if filter removes all
