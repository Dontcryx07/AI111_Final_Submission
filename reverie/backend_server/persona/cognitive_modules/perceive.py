...
def perceive(persona, maze): 
  ...
  # =============================================
  # UPDATE MOOD BASED ON PERCEIVED EVENTS (NEW)
  # =============================================
  mood_delta = 0.0
  for event_node in ret_events:
    desc_lower = event_node.description.lower()
    # Positive event keywords boost mood
    if any(w in desc_lower for w in ["enjoying", "finished", "completed", "happy",
                                      "fun", "relaxing", "excited", "playing",
                                      "laughing", "celebrating", "love"]):
      mood_delta += 0.05
    # Negative event keywords lower mood
    elif any(w in desc_lower for w in ["failed", "frustrated", "stressed", "lost",
                                        "argument", "lonely", "upset", "angry",
                                        "crying", "worried", "sick"]):
      mood_delta -= 0.05

  # Natural drift back toward baseline (mean reversion)
  drift = 0.02 * (persona.scratch.mood_baseline - persona.scratch.mood)

  # Apply mood change with clamping to [-1.0, 1.0]
  persona.scratch.mood = max(-1.0, min(1.0,
      persona.scratch.mood + mood_delta + drift))

  # Record mood for dashboard history
  if persona.scratch.curr_time:
    persona.scratch.mood_history.append(
        [persona.scratch.curr_time.strftime("%H:%M"),
         round(persona.scratch.mood, 3)])
    # Keep only last 500 entries to prevent memory bloat
    if len(persona.scratch.mood_history) > 500:
      persona.scratch.mood_history = persona.scratch.mood_history[-500:]
