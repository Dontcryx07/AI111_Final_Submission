...
def _long_term_planning(persona, new_day): 
  ...
  # =============================================
  # RESET DAILY COUNTERS (NEW)
  # =============================================
  persona.scratch.convo_count_today = 0

  wake_up_hour = generate_wake_up_hour(persona)

  # Initialize wake-up energy from sleep and current mood.
  # Sleep estimate: hours slept until wake-up (e.g., wake at 8 -> 8h sleep).
  hours_of_sleep = max(0, min(12, wake_up_hour))
  persona.scratch.energy_manager.initialize_wakeup_energy(
      hours_of_sleep=hours_of_sleep,
      wake_up_mood=persona.scratch.mood
  )
...
def _chat_react(maze, persona, focused_event, reaction_mode, personas):
  ...
  # =============================================
  # EMOTIONAL CONTAGION (NEW)
  # =============================================
  # When two agents talk, their moods pull toward each other.
  # This models the well-documented psychology phenomenon of emotional contagion.
  contagion_rate = 0.15  # 15% of mood difference transfers per conversation
  ...
  # =============================================
  # UPDATE RELATIONSHIP SCORES (NEW)
  # =============================================
  # Each conversation increases relationship score by 0.05, max 1.0
  if target_persona.name not in init_persona.scratch.relationship_scores:
    init_persona.scratch.relationship_scores[target_persona.name] = 0.0
  init_persona.scratch.relationship_scores[target_persona.name] = min(1.0,
      init_persona.scratch.relationship_scores[target_persona.name] + 0.05)

  if init_persona.name not in target_persona.scratch.relationship_scores:
    target_persona.scratch.relationship_scores[init_persona.name] = 0.0
  target_persona.scratch.relationship_scores[init_persona.name] = min(1.0,
      target_persona.scratch.relationship_scores[init_persona.name] + 0.05)
