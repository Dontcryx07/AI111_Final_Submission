...
class Scratch: 
  def __init__(self, f_saved): 
    ...
    # =============================================
    # MOOD / AFFECT SYSTEM (MODULARIZED)
    # =============================================
    ...
    if check_if_file_exists(f_saved): 
      ...
      # =============================================
      # LOAD MOOD / ENERGY FIELDS (MODULARIZED)
      # =============================================
      self.mood_manager.load_from_scratch(scratch_load, self.innate)
      self.energy_manager.load_from_scratch(scratch_load)

      # =============================================
      # DERIVE RETRIEVAL WEIGHTS FROM PERSONALITY (NEW)
      # =============================================
      if self.innate:
        innate_lower = self.innate.lower()
        # Nostalgic/sentimental people recall old memories more (lower recency weight)
        if any(t in innate_lower for t in ["nostalgic", "sentimental", "reflective", "traditional"]):
          self.recency_w = 0.5
        # Analytical/studious people prioritize important memories
        if any(t in innate_lower for t in ["analytical", "studious", "meticulous", "focused", "methodical"]):
          self.importance_w = 1.5
        # Social/outgoing people are more context-sensitive
        if any(t in innate_lower for t in ["outgoing", "social", "friendly", "empathetic", "warm"]):
          self.relevance_w = 1.5
  ...
  def save(self, out_json):
    ...
    # =============================================
    # SAVE MOOD / ENERGY FIELDS (MODULARIZED)
    # =============================================
    self.mood_manager.dump_to_scratch(scratch)
    self.energy_manager.dump_to_scratch(scratch)
  ...
  def get_str_iss(self): 
    ...
    # ORIGINAL CODE (COMMENTED OUT FOR VISIBILITY)
    # return commonset

    # =============================================
    # ADD MOOD TO IDENTITY STRING (NEW)
    # =============================================
    mood_label = self.mood_manager.get_mood_label()
    commonset += f"Current mood: {self.name} is currently feeling {mood_label} (mood score: {round(self.mood, 2)}).\n"

    return commonset


  # =============================================
  # MOOD COMPATIBILITY PROPERTIES (BACKWARD-COMPAT)
  # =============================================
  @property
  def mood(self):
    return self.mood_manager.mood

  @mood.setter
  def mood(self, value):
    self.mood_manager.mood = value

  @property
  def mood_baseline(self):
    return self.mood_manager.mood_baseline

  @mood_baseline.setter
  def mood_baseline(self, value):
    self.mood_manager.mood_baseline = value

  @property
  def mood_history(self):
    return self.mood_manager.mood_history

  @mood_history.setter
  def mood_history(self, value):
    self.mood_manager.mood_history = value

  @property
  def convo_count_today(self):
    return self.mood_manager.convo_count_today

  @convo_count_today.setter
  def convo_count_today(self, value):
    self.mood_manager.convo_count_today = value

  @property
  def relationship_scores(self):
    return self.mood_manager.relationship_scores

  @relationship_scores.setter
  def relationship_scores(self, value):
    self.mood_manager.relationship_scores = value

  @property
  def energy(self):
    return self.energy_manager.energy

  @energy.setter
  def energy(self, value):
    self.energy_manager.energy = value

  @property
  def energy_baseline(self):
    return self.energy_manager.energy_baseline

  @energy_baseline.setter
  def energy_baseline(self, value):
    self.energy_manager.energy_baseline = value

  @property
  def energy_history(self):
    return self.energy_manager.energy_history
