...
def dashboard(request, sim_code):
...
def api_metrics(request, sim_code):
  """API endpoint that returns current metrics as JSON."""
  metrics_file = f"storage/{sim_code}/metrics.json"
  if check_if_file_exists(metrics_file):
    with open(metrics_file) as json_file:
      data = json.load(json_file)
      return JsonResponse(data)
  return JsonResponse({"error": "No metrics available yet. Run the simulation first."})
